...
std::string JoinArgsRange(advancedfx::ICommandArgs* args, int startIndex, int endIndex) {
    std::string result;
...
std::string JoinArgs(advancedfx::ICommandArgs* args, int startIndex) {
    return JoinArgsRange(args, startIndex, args->ArgC());
}
    ...
    if(2 <= argC) {
        ...
        if(0 == stricmp("byUserId", arg1)) {
            ...
            if(3 <= argC) {
                const char * arg2 = args->ArgV(2);
                if(0 == stricmp("add", arg2) && 5 <= argC) {
                    const bool hasFlags = 6 <= argC;
                    const int valueEnd = hasFlags ? argC - 1 : argC;
                    unsigned int flags = hasFlags ? strtoul(args->ArgV(argC - 1),nullptr,10) : 17;
                    ...
                    result.first->second[flags] = JoinArgsRange(args, 4, valueEnd);
                    return;
                }
                else if(0 == stricmp("remove", arg2) && 4 <= argC) {
