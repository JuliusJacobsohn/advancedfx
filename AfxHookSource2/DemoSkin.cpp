...
bool patchItemView(unsigned char* itemView, const SkinRule& rule, uint64_t ownerXuid, int currentItemDef, bool allowItemDefWrite)
{
...
bool patchWeapon(CEntityInstance* weapon, SkinRule& rule, uint64_t ownerXuid, bool print)
{
	...
	*(float*)((unsigned char*)weapon + g_clientDllOffsets.C_EconEntity.m_flFallbackWear) = rule.wear;
	...
	patchItemView(itemView, rule, ownerXuid, currentDef, 0 < rule.itemDefIndex);
...
bool patchGloves(CEntityInstance* pawn, const SkinRule& rule, uint64_t ownerXuid, bool print)
{
	...
	if (!patchItemView(itemView, rule, ownerXuid, oldDef, true)) return false;
...
bool parseSkinRule(int startIndex, int argc, TArgs* args, SkinTarget target, SkinRule& rule)
{
	...
	for (int i = startIndex; i < argc; ++i) {
		...
		} else if ((value = getValueArg(arg, "statTrak")) || (value = getValueArg(arg, "stattrak"))) {
...
{
	advancedfx::Message(
		"%s xuid <steamid64> weapon active|all paintKit=<id> wear=<0..1> seed=<id> [statTrak=<id|off>] [defIndex=<weaponDef>] [itemDef=<itemDef>] [meshGroup=<mask>] - Configure weapon skin override.\n"
		"%s xuid <steamid64> gloves paintKit=<id> wear=<0..1> seed=<id> defIndex=<gloveItemDef> - Configure glove item view override.\n"
		"%s xuid <steamid64> clear - Clear one player's skin override.\n"
		"%s byXuid add x<steamid64> active|all paintKit=<id> wear=<0..1> seed=<id> [statTrak=<id|off>] [defIndex=<weaponDef>] [meshGroup=<mask>] - Backward-compatible weapon syntax.\n"
